...
[![C/ASM CI](https://github.com/kirill-bayborodov/bignum-bit-length/actions/workflows/ci.yml/badge.svg)](https://github.com/kirill-bayborodov/bignum-bit-length/actions/workflows/ci.yml)
[![GitHub release](https://img.shields.io/github/v/release/kirill-bayborodov/bignum-bit-length?label=release)](https://github.com/kirill-bayborodov/bignum-bit-length/releases/latest)

`bignum-bit-length` is a C11/x86-64 YASM module that counts significant bits in a fixed-capacity unsigned `bignum_t`. The production path scans the logical words from most significant to least significant and uses the highest-set-bit instruction for the final word under the System V AMD64 ABI. The input is read-only and the caller-provided output remains unchanged on error.
...
```bash
git clone --recurse-submodules https://github.com/kirill-bayborodov/bignum-bit-length.git
cd bignum-bit-length
```
...
- **Explicit API status:** the public API exposes `bignum_bit_length_status_t` rather than reusing a generic core status type.
- **Significant-bit query:** the highest non-zero logical word is located and its most significant set bit determines the result.
- **Transactional output:** validation completes before the significant-bit length is written.
- **Logical-prefix semantics:** physical words above `len` are ignored, including malformed dirty tails.
...
- **Bignum domain adapter:** `benchmarks/adapter/` maps generic transport fields to bignum-specific `bit-length` and operand-length semantics without relabelling them as unrelated operations.
- **Benchmark protocol:** successful runners print a machine-readable `benchmark=...` line immediately before `Benchmark finished.`.
...
The public API is declared in `include/bignum_bit_length.h` and returns the significant-bit count through a caller-provided output pointer:

```c
typedef enum bignum_bit_length_status {
    BIGNUM_BIT_LENGTH_SUCCESS = 0,
    BIGNUM_BIT_LENGTH_ERROR_NULL_ARG = -1,
    BIGNUM_BIT_LENGTH_ERROR_LENGTH = -2
} bignum_bit_length_status_t;

bignum_bit_length_status_t bignum_bit_length(
    const bignum_t *num,
    size_t *bit_length);
```
...
|---|---|---|
| Any required pointer is `NULL` | `BIGNUM_BIT_LENGTH_ERROR_NULL_ARG` | No object is dereferenced and output remains unchanged |
| Valid input with `len <= BIGNUM_CAPACITY` | `BIGNUM_BIT_LENGTH_SUCCESS` | Output receives the significant-bit count |
| Zero logical value | `BIGNUM_BIT_LENGTH_SUCCESS` | Output receives `0` |
| `num->len > BIGNUM_CAPACITY` | `BIGNUM_BIT_LENGTH_ERROR_LENGTH` | Output remains unchanged |

The operation is thread-safe when independent threads use non-overlapping objects. It does not modify either input and commits bit-length only after successful completion.
...
typedef enum {
    ...
    APPLICATION_STATUS_QUERY_ERROR = 1
} application_status_t;

application_status_t measure_value(const bignum_t *value, size_t *bits)
{
    bignum_bit_length_status_t status = bignum_bit_length(value, bits);

    return status == BIGNUM_BIT_LENGTH_SUCCESS
        ? APPLICATION_STATUS_SUCCESS
        : APPLICATION_STATUS_QUERY_ERROR;
}
```
...
| File | Scope |
|---|---|
| `tests/test_bignum_bit_length.c` | Deterministic API, contract, and boundary tests |
| `tests/test_bignum_bit_length_extra.c` | Extended state, preservation, and boundary checks |
| `tests/test_bignum_bit_length_mt.c` | Concurrent independent-object checks |
| `tests/test_bignum_bit_length_runner.c` | Distribution integration smoke test |
| `tests/benchmark_adapter/test_bignum_bit_length_benchmark_adapter.c` | C11 transport mapping, validation, deterministic initialization, operation, and checksum tests |
...
```text
benchmarks/bench_bignum_bit_length.c
benchmarks/bench_bignum_bit_length_mt.c
```
...
```text
benchmark=bignum_bit_length_st ... elapsed_seconds=<seconds> ns_per_call=<nanoseconds>
Benchmark finished.
```

The MT runner uses `benchmark=bignum_bit_length_mt`. The trailing marker is the success condition checked by the Makefile; it must remain after the machine-readable line.

| Mode | Input pattern | Purpose |
|---|---|---|
| `all_zero` | Every input `bignum_t` is zero; the bit-length operation receives zero-valued operands | Measures the zero-value fast path |
| `all_nonzero` | Inputs are populated through `BIGNUM_CAPACITY` words with nonzero top words | Measures the normal significant-bit query path |
| `mixed` | Alternating zero and nonzero input rows | Measures a mixed workload and branch behavior |

### Single-thread CLI

```text
bin/bench_bignum_bit_length \
  [--data-mode all_zero|all_nonzero|mixed] \
  [--input-kind zero|nonzero|mixed] \
  [--operation-kind bit-zero|bit-word|bit-random|bit-mixed] \
...
| `BENCH_OPERATION_KIND` | `bit-random` | `bit-zero`, `bit-word`, `bit-random`, or `bit-mixed` bignum operation transport value |
...
```text
bin/bench_bignum_bit_length_mt \
...
The reusable benchmark implementation is the public `v1.0.0` flat distribution under `libs/benchmark-framework/dist`. The project-local ST and MT sources include `benchmark_framework.h` and link `libbenchmark_framework.a`; matrix/statistics tools are consumed from `dist/tools/`. The adapter validates bignum vocabulary, constructs deterministic `bignum_t` records, chooses representable operand lengths, and maps `bignum_bit_length_status_t` to the named framework callback status.
...
`bench_matrix` invokes the pinned C11 `bench_matrix` and `benchmark_stats` tools directly, without Python or hardware PMU events. The default `benchmarks/profiles/bignum_bit_length_full.json` covers zero, one-word, multi-word and mixed operand cases, one/quarter/half/variable operand lengths, and safe near-capacity cases. `benchmarks/profiles/bignum_bit_length_standard.json` is the shorter bignum-specific smoke manifest and can be selected through `BENCH_MATRIX_PROFILE`. Each JSON manifest has a companion how-to document with its exact vocabulary and baseline workflow.
...
If the application requires symbols from the div-mod dependency graph, prefer the distribution created by `make dist CONFIG=release` and link the resulting static library with the corresponding component libraries.
